...
@Tag(name = "Dashboard and Reports", description = "Endpoints for querying factory and transport status")
public class EventLogController {
    ...
    @Operation(summary = "Get all event logs", description = "Returns the complete event sourcing log of all system activities.")
    ...
    @ResponseStatus(HttpStatus.OK)
    @Operation(summary = "Find event by ID", description = "Returns the details of a specific system event.")
    public EventLog getEventLogById(@PathVariable("reports_id") EventLogId id) {
        return eventLogService.findEventLogById(id);
    }

    @GetMapping("/stats")
    @Operation(summary = "Get global statistics", description = "Calculates real-time totals of processed, completed, and blocked orders, as well as trucks in transit.")
    ...
    @Operation(summary = "Get order history", description = "Returns the most recent status of each production order, unifying events by ID.")
